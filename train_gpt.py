...
def train_step(model, training_manager, train_loader, step):
    ...
    for idx in range(grad_accum_steps):
        ...
        training_manager.sparse_index_update(step, bigram_cpu)
        loss, tokens_numel = model(inputs, targets, cum_seqlens, bigram_inputs, training_manager.get_forward_args())
        loss += grad_scale
        training_manager.sparse_index_share(step)
...
def main():
    ...
    for step in range(train_steps + 1):
        ...
        print0(f"step:{step+1}/{train_steps}, train_time:{approx_training_time_ms/1000:.1f} secs, step_avg:{approx_training_time_ms/(step + 1):.2f}ms, train loss:{train_loss:.2f},  lr:{training_schedule.get_lr(step):.2f}", console=True)
